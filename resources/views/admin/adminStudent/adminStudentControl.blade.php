...
<div class="p-20 ">
    <main class=" bg-white  mx-auto p-8 md:p-6 my-0 rounded-lg shadow-2xl">
        ...
        <td><a href = 'editStudent/{{ $user->id }}' class="flex flex-col text-center py-2.5 px-4 rounded transition duration-200 hover:bg-blue-400 hover:text-white bottom-0 left-4">Edit</a></td>
        </tr>
        @endforeach
        </table>
        {{ $users->links() }}
        <div class="mt-10">
            <a href="{{ route('adminAddMember') }}" class=" py-2.5 px-4 rounded transition duration-200 hover:bg-green-400 hover:text-white bottom-0 left-4 ">Add a Student</a>
        </div>
    </main>
</div>
