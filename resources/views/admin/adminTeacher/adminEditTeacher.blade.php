...
<div class="p-20">
    <main class="bg-white  mx-auto p-8 md:p-6 my-0 rounded-lg shadow-2xl">
        <section>
            <h3 class="font-bold text-2xl">Edit Teacher</h3>
           {{-- {{  dd($user->id) }} --}}
        </section>
        <section class="mt-6">
            <form action="/edit/{{ $user->id }}" method="POST" class="flex flex-col">
                @csrf
            ...
            <form action="" class="flex flex-col">
                <a href="/deleteTeacher/{{ $user->id }}" class="text-center py-2.5 px-4 rounded transition duration-200 hover:bg-red-400 hover:text-white bottom-0 left-4">Delete Teacher</a>
            </form>
